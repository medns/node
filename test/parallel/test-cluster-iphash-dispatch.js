...
const concat = Array.prototype.concat.bind(Array);
...
if (cluster.isMaster) {
  ...
  const request = (port, callback) => {
    ...
    // Using all interfaces to request, simulate different ip sources.
    for (const { address } of interfaces) {
      ...
      client.on('data', common.mustCall((data) => {
        ...
        assert.strictEqual(typeof ip, 'string', 'ip must be string');
        assert.notStrictEqual(ip, '', 'ip must be not empty');
